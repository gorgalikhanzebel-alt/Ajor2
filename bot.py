...
async def search_user_callback(callback: types.CallbackQuery):
    if not await is_admin(callback.from_user.id):
        return await callback.answer("⛔ دسترسی!", show_alert=True)
    await callback.message.answer("🔍 لطفاً آیدی یا نام کاربر را بفرستید:")
    await callback.answer()
...
async def view_profile_callback(callback: types.CallbackQuery):
    if not await is_admin(callback.from_user.id):
        return await callback.answer("⛔ دسترسی!", show_alert=True)
    await callback.message.answer("👤 لطفاً آیدی عددی کاربر را بفرستید:")
    await callback.answer()
...
async def broadcast_callback(callback: types.CallbackQuery):
    if not await is_admin(callback.from_user.id):
        return await callback.answer("⛔ دسترسی!", show_alert=True)
    await callback.message.answer("📢 لطفاً پیام همگانی را بنویسید:")
    await callback.answer()
...
async def lock_unlock_callback(callback: types.CallbackQuery):
    if not await is_admin(callback.from_user.id):
        return await callback.answer("⛔ دسترسی!", show_alert=True)
...
async def ban_user_callback(callback: types.CallbackQuery):
    if not await is_admin(callback.from_user.id):
        return await callback.answer("⛔ دسترسی!", show_alert=True)
    await callback.message.answer("🚫 لطفاً آیدی کاربر را برای مسدود بفرستید:")
    await callback.answer()
...
async def unban_user_callback(callback: types.CallbackQuery):
    if not await is_admin(callback.from_user.id):
        return await callback.answer("⛔ دسترسی!", show_alert=True)
    await callback.message.answer("✅ لطفاً آیدی کاربر را برای رفع مسدود بفرستید:")
    await callback.answer()

@dp.callback_query(lambda c: c.data == "clear_messages")
async def clear_messages_callback(callback: types.CallbackQuery):
    if not await is_admin(callback.from_user.id):
        return await callback.answer("⛔ دسترسی!", show_alert=True)
    await callback.message.answer("🧹 لطفاً تعداد پیام‌ها را بفرستید:")
    await callback.answer()

# ============================================
# ======== توابع پردازش پیام ادمین ========
# ============================================
@dp.message(lambda msg: msg.text and msg.reply_to_message and "آیدی یا نام کاربر" in msg.reply_to_message.text)
...
@dp.message(lambda msg: msg.text and msg.text.isdigit() and msg.reply_to_message and "آیدی عددی کاربر" in msg.reply_to_message.text)
async def handle_profile_input(message: types.Message):
    if not await is_admin(message.from_user.id):
        return
...
@dp.message(lambda msg: msg.text and msg.reply_to_message and "پیام همگانی" in msg.reply_to_message.text)
async def handle_broadcast_input(message: types.Message):
    if not await is_admin(message.from_user.id):
        return
    text = message.text
...
@dp.message(lambda msg: msg.text and msg.text.isdigit() and msg.reply_to_message and "مسدود" in msg.reply_to_message.text)
async def handle_ban_input(message: types.Message):
    if not await is_admin(message.from_user.id):
        return
    user_id = int(message.text)
    if user_id == ADMIN_ID:
...
@dp.message(lambda msg: msg.text and msg.text.isdigit() and msg.reply_to_message and "رفع مسدود" in msg.reply_to_message.text)
async def handle_unban_input(message: types.Message):
    if not await is_admin(message.from_user.id):
        return
    user_id = int(message.text)
    if await is_banned(user_id):
...
@dp.message(lambda msg: msg.text and msg.text.isdigit() and msg.reply_to_message and "تعداد پیام‌ها" in msg.reply_to_message.text)
async def handle_clear_input(message: types.Message):
    if not await is_admin(message.from_user.id):
        return
