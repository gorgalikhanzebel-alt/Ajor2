...
# ======== لیست‌ها (کاملاً جدید و به عنوان پشتیبان هوش مصنوعی) ========
FUNNY_FALLBACKS = [
    "چی میگی بچه خوشگل؟ 😏",
...
JOKES = [
    ...
    "رفتیم رستوران، گارسون اومد گفت: میل دارید؟ گفتم: پ نه پ اومدیم صندلی‌ها رو تست کنیم! 😂",
    "‏میگن تو جهنم وقتی داری استراحت می‌کنی، یهو یکی میاد دسته دوم سفت و خراب پی‌اس‌فور رو میده بهت میگه بازی کنیم! 😂",
    "رفتم سوپرمارکت میگم چیپس دارید؟ میگه بله، میگم پس چرا تو قفسه نیست؟ میگه چون فروختمشون! 😂",
    "به بابام میگم برام آیفون می‌خری؟ می‌گه آره قفل فرمون هم براش می‌خرم که دزد نبره! 😂"
]

QUOTES = [
    "فرداهای روشن، سهم کسانی است که امروز برای رویاهایشان می‌جنگند. ✨",
    "موفقیت یک شبه به دست نمی‌آید؛ حاصل قدم‌های کوچکی است که هر روز برمی‌داری. 💪",
    "بزرگ‌ترین ریسک زندگی، ریسک نکردن است. مسیرت را خودت بساز! 🚀",
    "هرگز اجازه نده ترس‌هایت، جای رویاهایت را در زندگی بگیرند. 🌅",
    "بهترین زمان برای کاشتن یک درخت ۲۰ سال پیش بود، دومین زمان خوب همین الان است. 🌳"
]
...
async def joke_command(message: types.Message):
    ...
    if ai_joke:
        ...
        # استفاده از لیست جدید در صورت قطعی اینترنت ربات
