...
# ======== دیکشنری‌ها (سبک) ========
GREETINGS = {"سلام": "سلام! 👋", "خوبی": "خوبم ممنون! تو چطوری؟", "چطوری": "خوبم، ممنون!", "مرسی": "خواهش می‌کنم! 🤗", "خداحافظ": "خداحافظ! 👋", "صبح بخیر": "صبح بخیر! ☀️", "شب بخیر": "شب بخیر! 🌙", "خوش اومدی": "خوش اومدی! ✨", "چه خبر": "سلامت باشی! 😊", "درود": "درود بر تو! 🌹"}
JOKES = ["چرا مرغ از جاده رد شد؟ برای اینکه به اون طرف برسه! 😂", "بهترین زبان برنامه‌نویسی؟ پایتون! 🐍", "یک پنگوئن به یخچال نگاه کرد و گفت: چقدر خنک! 😄", "چرا ریاضیات غمگینه؟ چون مسائلش بی‌جوابه!", "چی می‌شه اگه نارگیل رو بندازی تو رودخونه؟ آب می‌شه!", "یک گربه به کامپیوتر گفت: منوس! 😹", "چرا برنامه‌نویس‌ها عاشق قهوه‌ان؟ ☕", "بهترین شوخی برنامه‌نویسی؟ null pointer exception! 😂"]
QUOTES = ["همیشه به فکر فردا باش!", "موفقیت یعنی بلند شدن دوباره!", "کد بزن و لذت ببر!", "زندگی مثل یه جعبه شکلاته!", "بهترین زمان برای شروع، الان است!", "هیچ چیز غیرممکن نیست، فقط زمان می‌بره! ⏳", "با امید و تلاش، قله‌ها فتح می‌شوند! 🏔️"]
FUNNY = ["چی میگی بچه خوشگل؟ 😏", "سیک تو بزن تا سیکمو نزدن 😂", "نمیفهمم حاجی چی میگی", "این چرت و پرتا چیه میگی مردک 🤔", "به نظرم ط ی چیزی زدی اینارو میگی"]

guess_games = {}

# ======== توابع ========
async def is_admin(user_id: int) -> bool: return user_id == ADMIN_ID
async def is_banned(user_id: int) -> bool: return banned_col.find_one({"_id": user_id}) is not None
...
def get_tehran_time():
    ...
    return utc + timedelta(hours=3, minutes=30)
...
async def start(message: types.Message):
    ...
    if await is_banned(user_id): return await message.answer("🚫 مسدود هستید!")
...
async def check_join(callback: types.CallbackQuery):
    if await is_banned(callback.from_user.id): return await callback.answer("🚫 مسدود هستید!", show_alert=True)
...
async def get_youtube(message: types.Message):
    ...
    try:
        ...
        if stream: await message.answer_video(stream.url, caption=f"🎬 {yt.title}")
        else: await message.answer("❌ خطا!")
    except: await message.answer("❌ لینک معتبر نیست.")
...
async def handle_guess(message: types.Message):
    ...
    elif guess < target: await message.answer(f"📈 بیشتر از {guess}")
    else: await message.answer(f"📉 کمتر از {guess}")
...
async def back(callback: types.CallbackQuery):
    if callback.data == "back_main": await callback.message.answer("🔙 منوی اصلی:", reply_markup=main_menu())
    else: await callback.message.answer("🔙 منوی بازی:", reply_markup=game_menu())
    await callback.answer()
...
async def admin_panel(callback: types.CallbackQuery):
    if not await is_admin(callback.from_user.id): return await callback.answer("⛔ دسترسی ندارید!", show_alert=True)
...
async def user_list(callback: types.CallbackQuery):
    ...
    if not users: return await callback.message.answer("📭 کاربری نیست.")
    text = "📋 ۲۰ کاربر اخیر:\n"
...
async def broadcast(callback: types.CallbackQuery):
    if not await is_admin(callback.from_user.id): return await callback.answer("⛔ دسترسی ندارید!", show_alert=True)
    await callback.message.answer("📢 پیام رو بفرست:")
    await callback.answer()

@dp.message(lambda msg: msg.text and await is_admin(msg.from_user.id))
async def handle_broadcast(message: types.Message):
    if message.reply_to_message and "پیام رو بفرست" in message.reply_to_message.text:
        text = message.text
...
async def ban_management(callback: types.CallbackQuery):
    ...
    text = "🚫 لیست مسدودها:\n" + "\n".join([f"ID: {b['_id']}" for b in banned[:20]]) if banned else "✅ هیچکس مسدود نیست."
    await callback.message.answer(text)
    await callback.answer()
...
async def ban_unban_cmd(message: types.Message):
    ...
    if await is_banned(user_id):
        ...
        if user_id == ADMIN_ID: return await message.answer("❌ خودت رو مسدود نکن!")
        banned_col.insert_one({"_id": user_id, "reason": "ادمین"})
        await message.answer(f"✅ {user_id} مسدود شد.")
        try: await bot.send_message(user_id, "🚫 توسط ادمین مسدود شدی!")
        except: pass
...
async def lock_unlock(callback: types.CallbackQuery):
    ...
    await callback.message.answer("🔒 قفل شد!" if can_send is False else "🔓 باز شد!")
    await callback.answer()
...
async def clear_cmd(message: types.Message):
    ...
    if count > 100: return await message.answer("❌ حداکثر ۱۰۰.")
    deleted = 0
...
async def upload_file_command(message: types.Message):
    if not await is_admin(message.from_user.id): return await message.answer("⛔ فقط ادمین!")
    await message.answer("📤 فایل رو بفرست:")

@dp.message(lambda msg: msg.document or msg.photo or msg.video)
async def handle_file_upload(message: types.Message):
    if not await is_admin(message.from_user.id): return await message.answer("⛔ فقط ادمین!")
    if message.document: file_id, file_type, name = message.document.file_id, "document", message.document.file_name
    elif message.photo: file_id, file_type, name = message.photo[-1].file_id, "photo", "عکس"
    elif message.video: file_id, file_type, name = message.video.file_id, "video", "ویدئو"
    else: return await message.answer("❌ پشتیبانی نمی‌شود.")
...
async def help_command(message: types.Message):
    await message.answer("📖 /start - شروع\n/time - زمان تهران\n/id - آیدی من\n/joke - جوک\n/quote - نقل قول\n/ping - وضعیت\n/admin - پنل ادمین\n/cancel - لغو حدس عدد")
...
async def admin_command(message: types.Message):
    if not await is_admin(message.from_user.id): return await message.answer("⛔ دسترسی ندارید!")
...
async def handle_text(message: types.Message):
    ...
    for key, response in GREETINGS.items():
        if key in text: return await message.answer(response)
    ai_response = await ask_ai(text)
    if ai_response: return await message.answer(ai_response)
